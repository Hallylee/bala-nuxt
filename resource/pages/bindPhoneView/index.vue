<template>
  <div>
    ...
    <commonBack :has-menu="false" page-type="h5_bind_phone" desc="back_top" />
...
export default {
  ...
  mounted() {
    statLoad('h5_bind_phone', 'bind_phone');
  },
  methods: {
    async sendCode() {
      const _self = this;
      ...
      try {
        UserService.bindPhine({
          ...
          if (res.code !== '0') {
            ...
            if (String(res.message).includes('绑定')) {
              this.$router.replace(`/mergeuid?phone=${this.phoneNumber}`);
            }
            return;
          }
